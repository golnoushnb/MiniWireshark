...
def info(destination_ip, n):
    ...
    dest_ip = inet_aton(destination_ip).hex() #dest io
    src_port = "%04x" %int(Lines[3]) #src port
    dest_port ="%04x" %int(n) #dest port
    ...
    pkt_sender(eth_frame, interface0)



destination_ip = input("Enter a destination ip: ")
i, j = [int(x) for x in input("Enter the range of ports: ").split()]
for n in range(i, j):
    info(destination_ip, n)
    print("sent tcp syn packet to port ", n)
